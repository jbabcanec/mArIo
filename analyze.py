...
from analyze_direction import DirectionAnalyzer
from analyze_lap import LapAnalyzer
...
class ScreenAnalyzer:
    ...
    def analyze(self, screenshot):
        ...
        direction_output = self.direction_analyzer.get_direction()
        lap_output = self.lap_analyzer.get_lap(screenshot_np)
        ...
        cv2.putText(canvas, f'Direction: {direction_output}', (10, 150), font, 0.7, color, thickness, cv2.LINE_AA)
        cv2.putText(canvas, f'Lap: {lap_output}', (10, 180), font, 0.7, color, thickness, cv2.LINE_AA)
